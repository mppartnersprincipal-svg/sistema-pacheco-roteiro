...
export async function downloadMultipleScriptsPDF(items: Array<{ script: Script; review?: Review; tema: string }>) {
  const { jsPDF } = await import('jspdf')
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' })
  ...
  doc.setFontSize(16)
  doc.setFont('helvetica', 'bold')
  doc.text('PachecoSolar — Roteiros Pre-Aprovados', margin, 14)
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`${items.length} roteiro(s) — ${new Date().toLocaleDateString('pt-BR')}`, margin, 24)
  y = 40

  doc.setFontSize(10)
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(30, 30, 30)
  doc.text('Roteiros incluidos:', margin, y)
  y += 7
  items.forEach((item, i) => {
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(50, 50, 50)
    doc.text(`${i + 1}. ${sanitize(item.tema)}`, margin + 4, y)
    y += 5
  })
  y += 5
  divider()
  ...
  for (const item of items) {
    ...
    // Separador visual entre roteiros
    doc.setFillColor(249, 115, 22)
    doc.rect(0, 0, W, 18, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(12)
    doc.setFont('helvetica', 'bold')
    doc.text('PachecoSolar — Roteiro de Video', margin, 12)
    y = 26

    title(script.tema, 13)
    y += 2
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(100, 100, 100)
    ...
    if (review) {
      ...
      scores.forEach(([label, val]) => { body(`${label}: ${val}/10`, 0) })
      y += 3
      divider()
  ...
  doc.save(`roteiros-pre-aprovados-${date}.pdf`)
}
